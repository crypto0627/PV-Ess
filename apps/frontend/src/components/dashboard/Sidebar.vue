<template>
  <aside
    class="absolute top-6 left-6 bottom-11 flex-col gap-2 w-[260px] rounded-2xl p-4 bg-white/10 backdrop-blur-[30px] border border-white/10 shadow-lg transition-all duration-400 hover:shadow-xl z-20 md:block"
...
const handleSubItemClick = async (subItem) => {
  ...
  try {
    await router.push(subItem.link || '/main/settings')
    if (window.innerWidth < 768) {
      isSidebarOpen.value = false
    }
  } catch (error) {
